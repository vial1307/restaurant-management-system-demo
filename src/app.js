...
import { attachBusinessStateSync } from "./business-state-sync.js?v=91";
import {
  activeInventorySite,
...
function quantityControl(item, kind = "item", manageAdjust = false) {
  ...
  const value = direct
    ? numberInput(item.quantity, `data-field="${kind}" data-key="quantity" data-id="${escapeHtml(item.id)}"${manageAttribute}`, "quantity-input")
...
root.addEventListener("submit", async (event) => {
  ...
  if (["add-item", "edit-item"].includes(form.dataset.form)) {
    ...
    const site = activeInventorySite();
    const submitButton = form.querySelector("[data-save-item]");
    if (submitButton) {
      submitButton.disabled = true;
      submitButton.textContent = "Đang lưu vào database… · 正在儲存…";
    }
    view.modal = null;

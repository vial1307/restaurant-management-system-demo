...
-- Authoritative reconciliation of the user-supplied Fuxing 大冷凍 inventory
-- confirmed on 2026-09-11. PostgreSQL is the source of truth.
-- Scope is intentionally limited to fuxing-large-freezer; stock in other
-- branch storage/work locations is not zeroed or inferred.
do $stocktake$
declare
  v_location_id uuid;
  v_item_id uuid;
...
begin
  ...
  where code='fuxing-large-freezer'
    and site='fuxing'
    and kind='storage'
    and active=true
  limit 1;
  ...
  if v_location_id is null then
    raise notice 'Skipping Fuxing 2026-09-11 reconciliation: canonical production location not present.';
    return;
  end if;

  for v_row in
    select * from (values
      ('fuxing:freezer-beef-noodle-broth','牛麵湯','Nước dùng mì bò','包','soup',20::numeric,'8/31:5+3000; 9/1:5+3720; 9/4:5+3980; 9/8:5+3590 (20包 + 14290g散量)'),
      ('fuxing:freezer-clear-stew-broth','清燉湯','Nước dùng bò hầm trong','包','soup',20,'9/1:5+2480; 9/1:5+3010; 9/3:5+3370; 9/8:5+2920 (20包 + 11780g散量)'),
      ...
      ('fuxing:freezer-light-mala-broth','輕麻湯包','Gói nước dùng mala nhẹ','包','soup',78,'78袋'),
      ('fuxing:freezer-heavy-mala-broth','重麻湯包','Gói nước dùng mala đậm','包','soup',67,'16+51 = 67袋'),
      ('fuxing:oxtail-rice','牛尾追飯','Cơm đuôi bò','包','noodles',91,'91'),
      ('fuxing:freezer-oxtail-meat-2kg','牛尾肉袋(2K/包)','Túi thịt đuôi bò 2 kg','包','soup',9,'9包（2K/包），其中6包生'),
      ...
      ('fuxing:freezer-braised-tofu','滷膠豆干','Đậu phụ khô kho 滷膠','包','noodles',105,'105'),
      ('fuxing:freezer-braised-duck-wing','滷膠鴨翅（剁）','Cánh vịt kho 滷膠 (chặt)','包','noodles',57,'57（剁）'),
      ('fuxing:freezer-braised-duck-tongue','滷膠鴨舌','Lưỡi vịt kho 滷膠','包','noodles',54,'54'),
      ...
      ('fuxing:freezer-rice-cake','追飯糕','Bánh 追飯糕','包','meat',26,'8/10:26'),
      ('fuxing:freezer-tender-beef','滑牛','Thịt bò mềm ướp','包','meat',17,'17'),
      ...
      ('fuxing:freezer-frog','田雞','Thịt ếch','包','seafood',31,'31'),
      ('fuxing:freezer-large-intestine','大腸','Lòng già','包','seafood',40,'40'),
      ('fuxing:freezer-braised-tripe','滷牛肚','Dạ dày bò kho','包','seafood',110,'110'),
      ...
      ('fuxing:freezer-hell-tripe','地獄牛肚','Dạ dày bò địa ngục','包','noodles',24,'24'),
      ('fuxing:freezer-rice-sauce-180g','地獄燴飯180克','Sốt cơm địa ngục 180 g','包','noodles',28,'28（地獄燴飯180克）'),
      ('fuxing:freezer-hell-beef-rice','地獄牛肉燴飯','Cơm sốt thịt bò địa ngục','包','noodles',18,'18'),
      ...
      ('fuxing:freezer-fried-taro','炸芋頭','Khoai môn chiên','包','soup',3,'3'),
      ('fuxing:freezer-fried-squid','炸魷魚','Mực chiên','包','soup',1,'1'),
      ('fuxing:freezer-buniu-concentrate','不牛濃縮','Cốt cô đặc 不牛','包','soup',3,'3'),
      ('fuxing:freezer-sous-vide-chicken','舒肥雞','Gà sous-vide','包','soup',18,'18包 + 44片 + 96片（未提供包/片換算；主庫存保留18包，另140片保留於盤點明細）'),
      ('fuxing:freezer-pork-knuckle','豬腳','Chân giò heo','包','soup',6,'6'),
      ('fuxing:freezer-sous-vide-pork-shoulder','舒肥梅花豬','Nạc vai heo sous-vide','包','soup',5,'5'),
  ...
  loop
    ...
    if v_before is distinct from v_row.quantity then
      insert into public.inventory_transactions(
        ...
        '復興大冷凍權威同步 2026-09-11 / Đồng bộ chuẩn tủ đông lớn Fuxing 2026-09-11',
        null,'system:stocktake-2026-09-11',
        jsonb_build_object(
          'source','user_confirmed_reconciliation',
          'site','fuxing',
          'location_code','fuxing-large-freezer',
          'counted_at','2026-09-11',
          'raw_detail',v_row.raw_detail,
          'before_quantity',v_before,
          'after_quantity',v_row.quantity,
          'unit',v_row.unit,
  ...
  insert into public.audit_logs(
    ...
    null,'system:stocktake-2026-09-11','stocktake_reconcile','inventory_location',
    v_location_id::text,'fuxing',null,
    jsonb_build_object('rows',v_applied,'location_code','fuxing-large-freezer'),
    jsonb_build_object(
      'source','user_confirmed_reconciliation',
      'counted_at','2026-09-11',
      'location_assertion','Fuxing -> 大冷凍 only',
      'mixed_unit_policy','configured stock unit retained; residual grams/pieces preserved in transaction raw_detail where no conversion factor was supplied'
    )
  );
end
$stocktake$;

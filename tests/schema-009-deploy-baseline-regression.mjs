...
const migrationUrl = new URL("vps/database/migrations/009_fuxing_large_freezer_reconcile_20260911.sql", root);
...
assert.match(migrate, /if \[\[ "\$\{applied\}" == "1" \]\]; then[\s\S]*?skip \$\{base\}/, "applied migrations must be skipped instead of re-running one-time reconciliation");

assert.match(verify, /if \[\[ "\$\{schema\}" < "009" \]\]; then/, "production verifier must reject schemas older than 009");
assert.match(verify, /older than 009/, "production verifier error text must identify schema 009 as the minimum");

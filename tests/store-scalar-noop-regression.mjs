...
try {
  ...
  const assertNoop = (label, action) => {
    ...
    action();
    assert.deepEqual(snapshotSignals(), before, `${label} must not persist, notify, bump updatedAt or increment pendingSync`);
  };

  const assertRealChange = (label, action) => {
    const before = snapshotSignals();
    action();
    const after = snapshotSignals();
  ...
  const selectedDateBeforeRepeat = store.getState().selectedDate;
  assertNoop("same selected date", () => store.selectDate(selectedDateBeforeRepeat));
  assert.equal(store.getState().selectedDate, selectedDateBeforeRepeat, "same-date no-op must preserve selected service date");
  ...
  assert.ok(linkedStorage, "linked inventory/work pair is required for propagation regression");
  ...
  assert.equal(linkedWork.label, authoritativeLabel, "same primary label must still repair a stale linked work row");
  ...
  assert.equal(linkedStorage.workArea, authoritativeArea, "same work-item area must still repair stale linked storage rows");

  console.log("STORE_SCALAR_NOOP_REGRESSION_OK");
} finally {

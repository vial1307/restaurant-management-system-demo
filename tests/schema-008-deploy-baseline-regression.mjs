...
const currentMigrationUrl = new URL("vps/database/migrations/015_operational_master_data.sql", root);
...
assert.equal(fs.existsSync(currentMigrationUrl), true, "current production migration 015 must exist in the canonical repository");
...
assert.match(verify, /if \[\[ "\$\{schema\}" < "015" \]\]; then/, "production verifier must reject schemas older than 015");
assert.match(verify, /older than 015/, "production verifier error text must identify schema 015 as the minimum");
assert.doesNotMatch(verify, /if \[\[ "\$\{schema\}" < "00[78]" \]\]; then/, "stale pre-Core-v2 production baseline must not return");

console.log("SCHEMA_015_DEPLOY_BASELINE_OK");

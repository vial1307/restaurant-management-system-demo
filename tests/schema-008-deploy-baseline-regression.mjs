...
const currentMigrationUrl = new URL("vps/database/migrations/024_inventory_location_archive_integrity.sql", root);
...
assert.equal(fs.existsSync(currentMigrationUrl), true, "current production migration 024 must exist in the canonical repository");
...
assert.doesNotMatch(
  ...
  "runtime/backend regression wrappers must not pin retired schemas 020/021/022/023"
);

console.log("SCHEMA_024_DEPLOY_BASELINE_OK");

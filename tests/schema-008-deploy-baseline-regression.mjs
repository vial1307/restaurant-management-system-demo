...
const currentMigrationUrl = new URL("vps/database/migrations/023_inventory_archive_integrity.sql", root);
...
assert.equal(fs.existsSync(currentMigrationUrl), true, "current production migration 023 must exist in the canonical repository");
...
assert.match(verify, /if \[\[ "\$\{schema\}" < "023" \]\]; then/, "production verifier must reject schemas older than 023");
assert.match(verify, /older than 023/, "production verifier error text must identify schema 023 as the minimum");
...
assert.doesNotMatch(
  ...
  "runtime/backend regression wrappers must not pin retired schemas 020/021/022"
);

console.log("SCHEMA_023_DEPLOY_BASELINE_OK");

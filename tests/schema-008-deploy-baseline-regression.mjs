...
assert.equal(fs.existsSync(migrationUrl), true, "historical production migration 008 must remain in the canonical repository");
...
assert.match(migrate, /if \[\[ "\$\{applied\}" == "1" \]\]; then[\s\S]*?skip \$\{base\}/, "applied migrations must be skipped instead of re-running historical stocktakes");

console.log("SCHEMA_008_HISTORICAL_GUARD_OK");

...
assert.match(ui,/request\("\/api\/admin\/super\/sites"\)/);
assert.match(ui,/payload\?\.siteRegistryChanged/);
...
await emit("/api/master-data/locations");await emit("/api/master-data/work-areas");await emit("/api/admin/super/data/:dataset",200,"POST",{dataset:"inventory-products"});await emit("/api/admin/super/inventory-catalog-identity");await emit("/api/admin/super/sites");
assert.equal(events.length,start+5);
assert.match(events.at(-1),/"siteRegistryChanged":true/);

...
assert.equal(deferredPreferenceWrites[0].preferredLanguage, "zh-TW");
assert.equal(JSON.parse(storage.get(PENDING_LANGUAGE_KEY) || "null")?.preferredLanguage, "vi", "latest language intent did not replace the older pending value");
...
assert.equal(deferredPreferenceWrites[1].preferredLanguage, "vi", "coalesced follow-up write did not use the latest language intent");
...
await delay(20);
assert.equal(serverPreferredLanguage, "vi", "serialized preference writes did not leave the server on the latest language");
